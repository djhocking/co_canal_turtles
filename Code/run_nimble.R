...
psi_sex_tmp <- runif(1, 0.3, 0.8)
sex[is.na(sex)] <- rbinom(length(sex[is.na(sex)]), 1, psi_sex_tmp)
...
parameters <- c("density", "N", "alpha2", "alpha0", "alpha1", "mu_0", "sd_0", "beta_0", "beta_1", "beta_2", "beta_3", "psi_sex", "p_cap_day", "sigma_mean", "sigma", "p_cap_site") ## "p_site_ind", "sigma", # "C", maybe C or a summary stat, might blow up if saving each activity center "s".  "mu_1", "sd_1", "mu_psi", "sd_psi",
...
samples <- nimbleMCMC(
  ...
  clusterSetRNGStream(cl = cl, 3453469)
