...
testing <- TRUE # use separate file that can call from bash script
run_date <- "2019-09-28" # check output and input to file that can just read in the most recent

######### Load Data from Previous script #########

if(testing) {
  load(file = paste0("Data/Derived/prub_testing_", run_date, ".RData"))
} else {
  load(file = "Data/Derived/prub.RData")
}
...
# in previous code add max(EM_array[ , , , ], na.rm = TRUE) for each species and save as max_ind_sp (683 for PRUB)
...
saveRDS(out, "Results/JAGS/prub_mcmc_test.rds")
saveRDS(out, "Results/JAGS/prub_mcmc_final.rds")
